...
export const Page = () => {
  ...
  const [uiState, setUiState] = useState<
    'loading' | 'configure' | 'building' | 'built' | 'uploading' | 'uploaded' | 'request-rebuild'
  >('loading');
  ...
  useEffect(() => {
    ...
    // Special case: if we have a ready snapshot that hasn't been uploaded yet, and the user wants to rebuild it,
    // we know that the newly created snapshot will have a different uid from the current state, so we can transition
    // to the building spinner state. Once the newly created snapshot matches the state, it is ready to be uploaded again.
    if (
      uiState === 'request-rebuild' &&
      createSnapshotResult.data?.uid !== snapshot.data?.uid &&
      snapshotStatus === 'PENDING_UPLOAD'
    ) {
      setUiState('building');
      return;
    }
    ...
    // When we have an uploaded snapshot, and we want to reupload it again from the final screen.
    if (uiState === 'uploaded' && (snapshotStatus === 'PROCESSING' || snapshotStatus === 'UPLOADING')) {
      setUiState('uploading');
      return;
    }
  }, [
  ...
  console.log('different', createSnapshotResult.data?.uid !== snapshot.data?.uid);
  ...
  const handleCreateSnapshot = useCallback(
    ...
    [performCreateSnapshot, sessionUid, snapshotStatus]
  );
  ...
  }, [setUiState, sessionUid, snapshotUid]);
  ...
  return (
    <>
      <Stack direction="column" gap={2}>
        ...
        {(uiState === 'uploaded' || uiState === 'building' || uiState === 'built') && error && (
